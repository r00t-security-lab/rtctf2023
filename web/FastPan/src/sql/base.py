...
async_scoped_session: ContextVar[Optional[AsyncSession]] = ContextVar(
    'async_scoped_session', default=None)

engine = create_async_engine(get_sql_url(), echo=True, future=True)
SessionMaker = async_sessionmaker(
    bind=engine, class_=AsyncSession, future=True, expire_on_commit=False)
...
def get_session(**kwargs) -> AsyncSession:
    ...
    assert isinstance(
        sess, AsyncSession), f'async_scoped_session.get() returned {type(sess)} but not AsyncSession'
    # print(id(sess))
    return sess
